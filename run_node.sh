...
./target/release/dock-testnet --dev --chain=remdev --rpc-external --ws-external --rpc-cors=all &

# Sleep to ensure node has started. This is a temporary hack. The right solution would be to check node status in a loop
sleep 15

# The node has 1 authority. Create keys for it. The secret phrase, auroa public key and grandpa public key are passed as command line argument,
