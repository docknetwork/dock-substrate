use jsonrpsee::{
    core::{async_trait, Error, RpcResult as Result},
    proc_macros::rpc,
...
pub trait BasicChannelApi {
    ...
    async fn get_merkle_proof(&self, commitment_hash: H256, leaf_index: u64) -> Result<Bytes>;
}
...
mod tests {
    ...
    fn basic_channel_rpc_should_create_proof_for_existing_commitment() {
        ...
        let result = futures::executor::block_on(rpc_handler.get_merkle_proof(TEST_HASH.into(), 0))
    ...
    fn basic_channel_rpc_should_handle_leaf_index_out_of_bounds() {
        ...
        let rpc_handler = create_rpc_handler(
            sp_offchain::STORAGE_PREFIX,
            TEST_HASH,
            Some(leaves.encode()),
        );

        let result = futures::executor::block_on(rpc_handler.get_merkle_proof(TEST_HASH.into(), 2));
