...
decl_module! {
	pub struct Module<T: Trait> for enum Call where origin: T::Origin {
		...
		/// Deposit balance from EVM into currency/balances module.
		#[weight = 0]
		fn deposit(origin, address: H160, value: BalanceOf<T>) {
			// TODO: Introduce `DepositOrigin` to be replace `WithdrawOrigin`
			let source = T::WithdrawOrigin::ensure_address_origin(&address, origin)?;
